...
/*   Updated: 2021/09/09 14:36:44 by fgradia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_eat_sleep_think(t_philo *actual)
{
	printf("%ld eat_sleep_think\n", actual->name);

	// printf("%ld eat_sleep_think\teat %ld\n", actual->name, actual->eat_n);
	actual->eat_n--;
}
...
void	ft_routine(t_philo	*philos, int actual)
{
	while (philos[actual].eat_n != 0)// && philos[actual].data->die_all != 666)
		ft_eat_sleep_think(&philos[actual]);
	free(philos);
	exit(0);
}
